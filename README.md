...
Después de instalar, abre el archivo **`.env`** con cualquier editor de texto (Bloc de notas, VS Code, etc.) y rellena las dos credenciales necesarias:
...
~12.000 licitaciones
        ...
        │
        ▼  ~301 licitaciones finales (~92 recuperadas por bypass)
```

Los tres conjuntos de palabras clave se configuran en el archivo `config_pivot/PIVOT_MAESTRO.xlsx`, hoja `06-FILTROS`. **No se necesita tocar el código para ajustar los filtros.**
...
| Columna | Índice | Contenido | Cuándo usar |
|---------|--------|-----------|-------------|
| A | 0 | Inclusión — NOMBRE | Palabras que deben aparecer en el nombre de la licitación |
| B | 1 | Inclusión — NIVEL1 | Categorías de nivel 1 que incluir |
| C | 2 | Inclusión — NIVEL2 | Categorías de nivel 2 que incluir |
| D | 3 | Inclusión — NIVEL3 | Categorías de nivel 3 que incluir |
| E | 4 | Exclusión — NOMBRE | Palabras en el nombre que causan descarte |
| F–L | 5–11 | Exclusión — otros campos | Exclusión por nivel, genérico, organismo, tipo |
| M | 12 | BYPASS | Palabras que recuperan una licitación aunque haya sido excluida |
...
```bash
# Los mismos que el proyecto + pytest ya está en requirements.txt
```
...
class MiEtapa(BaseStage):
    ...
    def validate_inputs(self, context: PipelineContext) -> bool:
        # Verificar que los artefactos necesarios existan en context
        return True
...
Cada etapa comunica sus resultados a través de `PipelineContext`:
...
context.flags['areas_seleccionadas'] = ['TI']
```
...
### Modificar los filtros desde código (no desde Excel)
